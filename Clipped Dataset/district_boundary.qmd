<!DOCTYPE qgis PUBLIC 'http://mrcc.com/qgis.dtd' 'SYSTEM'>
<qgis version="3.9.0-Master">
  <identifier></identifier>
  <parentidentifier></parentidentifier>
  <language></language>
  <type>dataset</type>
  <title></title>
  <abstract>This is a polygon of a district namely Surabaya

1. InaRISK raster Flood Risk service (WCS)
2. InaRISK PDF report of Hazard Risk, RBI 2013
3. InaRISK PDF report of Hazard Risk, RBI 2016

We are trying to select districts with high exposure rate: 
The exposure rate class is given in field: 'CANDIDATE_CLASS' with the following value:

> 100.000 people exposed: 'PRIMARY'
and
100.000 > x > 50.000 people exposed: 'SECONDARY'

and correlate it with list of high risk from InaRISK raster Flood Risk service.
The raster files are classified into a zonal histogram for each administrative boundary of districs.
The zonal histograms counts into the following field:

histo_NODATA: for pixel index below 0.6
histo_70: for pixel index between 0.6 - 0.7
histo_80: for pixel index between 0.7- 0.8
histo_90: for pixel index between 0.8 - 0.9

no histo_100 field because there are no pixel index above 0.9

The field total_count contains the total pixel in that district
The field score is calculated according to the formulae:  score = histo_NODATA * 0 + histo_70 * 0.7 + histo_80 * 0.8 + histo_90 * 0.9
The field ratio is used as normalized value of score, using the formula: ratio = score/total_count


The field KABUPATEN, is the name of the district.</abstract>
  <contact>
    <name></name>
    <organization></organization>
    <position></position>
    <voice></voice>
    <fax></fax>
    <email></email>
    <role></role>
  </contact>
  <links/>
  <fees></fees>
  <encoding></encoding>
  <crs>
    <spatialrefsys>
      <proj4></proj4>
      <srsid>0</srsid>
      <srid>0</srid>
      <authid></authid>
      <description></description>
      <projectionacronym></projectionacronym>
      <ellipsoidacronym></ellipsoidacronym>
      <geographicflag>false</geographicflag>
    </spatialrefsys>
  </crs>
  <extent>
    <spatial maxx="0" minz="0" crs="" dimensions="2" miny="0" minx="0" maxy="0" maxz="0"/>
    <temporal>
      <period>
        <start></start>
        <end></end>
      </period>
    </temporal>
  </extent>
</qgis>
